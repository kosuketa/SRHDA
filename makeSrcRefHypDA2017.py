...
DIR=HOME+'/script'
DataDir=HOME+'/data/wmt17-submitted-data/txt'
HypDir=DataDir+'/system-outputs/newstest2017'

languages = ['cs-en', 'de-en', 'fi-en', 'lv-en', 'ru-en', 'tr-en', 'zh-en']
...
for lang in languages:
    systems[lang] = [re.sub(r'(newstest2017\.)|\.'+lang, '', s) for s in listdir(HypDir+'/'+lang)]
    hyps[lang] = {}
    
    for f in listdir(HypDir+'/'+lang):
        for system in systems[lang]:
            fpattern = 'newstest2017.'+system+'.'+lang
            if re.match(fpattern, f):
...
DAfiles = {}
for lang in languages:
    DAfiles[lang]= HOME+'/data/DAsys-wmt-newstest2017/anon-proc-hits-sys-combined/analysis/' +          'ad-seg-scores-' + lang + '.csv'    
...
for lang in languages:
    DF[lang] = makeDF(lang)
    saveDf2Csv(WriteDir+'/newstest2017.'+lang+'.csv', DF[lang])
...
def combineData(lang):
    ...
    return pd.concat([pd.concat([loadCsv2Df(data2017).drop(['RawDA'],axis=1), loadCsv2Df(data2016)], axis=0), loadCsv2Df(data2015)], axis=0)
